...
function handleSubmit() {
  ...
  const tensanpham = document.getElementById("giasanpham").value
  ...
  const products = {tensanpham,masanpham,soluong,hinhanh,giasanpham}
  const data = getDataToStore();
  if (btn_action == "create") {
    if (ktNameTonTai(tensanpham)) {
  ...
  document.getElementById("masanpham").readOnly = false;
  setDataInStore("products", data)
  createTable()
  return false
}

function ktNameTonTai(masanpham) {
  let data = getDataToStore()
  for (let i = 0; i < data.length; i++) {
    if (data[i].masanpham == masanpham) {
